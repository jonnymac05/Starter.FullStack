...
if (launchHttpServer) {
  ...
  app._server = http.listen(publicPort, () => {
    const url = `listening on http://localhost:${publicPort}${apiPrefix}api/ping  {click here to ping}`;
    console.log(url);
  });
}
...
if (launchSecureServer === true && _httpsServer) {
  ...
  app._secureServer = _httpsServer.listen(securePort, () => {
    const url = `listening on https://localhost:${securePort}${apiPrefix}api/ping  {click here to ping}`;
    console.log(url);
  });
}
